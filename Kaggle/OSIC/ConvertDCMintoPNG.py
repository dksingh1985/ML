...
dest = "D:\\Data\\OSIC\\osic-pulmonary-fibrosis-progression\\png"

#check destination file 
if os.path.isdir(dest) == False:
    os.mkdir(dest)

# process each patients data
for k in range(train.shape[0]):
    
    if os.path.isdir(dest + "\\" + train.iloc[k,0] ) == False:
        os.mkdir(dest + "\\" + train.iloc[k,0])

    source_dir = director + "\\" + train.iloc[k,0]
    dest_dir = dest + "\\" + train.iloc[k,0] 
    
    print(k, " --------> ", source_dir)

    files = []
    for dcm in list(os.listdir(source_dir)):
        files.append(dcm) 
    files.sort(key=lambda f: int(re.sub('\D', '', f)))

    # Read in the Dataset
    for dcm in files:
        source_path = source_dir + "\\" + dcm
        dest_path = dest_dir + "\\" + dcm + ".png"
        try:
            img1 = pydicom.dcmread(source_path).pixel_array
