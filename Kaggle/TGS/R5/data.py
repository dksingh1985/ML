...
IMAGE_SIZE = 112
ACTUAL_SIZE = 101
Z_DF = pd.read_csv("D:\\Data\\Kaggle\\TGS\\depths.csv")
...
def imgPredictNpy2(image_path,imgs,indx,batch_size):
    outs = np.zeros((batch_size,IMAGE_SIZE,IMAGE_SIZE,1))
    for i in range(batch_size):
        img = Image.open(image_path + "\\" + imgs[(indx*batch_size) + i]).convert("L")
        img = img.resize((IMAGE_SIZE,IMAGE_SIZE), Image.ANTIALIAS)
        img = np.divide(img,255)
...
def saveResult2(save_path,imgs,nparray,indx,batch_size):
    for i in range(batch_size):
        img = nparray[i]
        img[img > 0.5] = 1
        img[img <= 0.5] = 0
        ...
        img = Image.fromarray(img.astype(np.uint8))
        img = img.resize((ACTUAL_SIZE,ACTUAL_SIZE),Image.ANTIALIAS)
        img.save(save_path + "\\" + imgs[(indx*batch_size) + i])
...
def prepareData(image_path,file_name):
    img = Image.open(image_path + "\\" + file_name)
    img = np.array(img)
    img[:,:,1] = np.multiply(np.divide(Z_DF[Z_DF.id == file_name.split(".")[0]].iloc[0].z,1000),255)
    img[:,:,2] = 0
    #print(img.shape)
