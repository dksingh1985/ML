...
def unet2(pretrained_weights = None,input_size = (96,96,3)):
    ...
    #Bottom Layer 6, 3
    bl_conv_6 = Conv2D(2048, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(dl_pool_5)
    ...
    #model.compile(optimizer=Adam(lr = 1e-4), loss='binary_crossentropy', metrics=['accuracy'])
    model.compile(loss="binary_crossentropy", optimizer=SGD(lr=0.0001,momentum=0.01),metrics=["accuracy"])
