...
tareget_cost = 0.1
learning_rate = 1
model_save_path = "D:\\Data\\Kaggle\\TGS\\Model_R4"
...
l2_f_w = tf.Variable(tf.truncated_normal([910,10201],0.5,0.25),name="Weight-2")
l3_f_w = tf.Variable(tf.truncated_normal([26*26*l2_filter_count,10201],1,0.5),name="Weight-3")
l4_f_w = tf.Variable(tf.truncated_normal([10201,10201],1,0.5),name="Weight-4")
l5_f_w = tf.Variable(tf.truncated_normal([10201,10201],1,0.5),name="Weight-4")

l1_c_b = tf.Variable(tf.truncated_normal([l1_filter_count],1,0.5),name="Bias-Conv1")
l2_c_b = tf.Variable(tf.truncated_normal([l2_filter_count],1,0.5),name="Bias-Conv2")
l3_f_b = tf.Variable(tf.truncated_normal([10201],1,0.5),name="Bias-3")
l4_f_b = tf.Variable(tf.truncated_normal([10201],1,0.5),name="Bias-4")
l5_f_b = tf.Variable(tf.truncated_normal([10201],1,0.5),name="Bias-4")
...
Layer1_out_a_dp = tf.nn.dropout(Layer1_out_a,keep_prob=keep_prob)

#Layer 2
Layer2_out_f = tf.nn.conv2d(input=Layer1_out_a_dp,filter=l2_c_f,strides=[1,1,1,1],padding="SAME") + l2_c_b
...
layer_flat_dp = tf.nn.dropout(layer_flat,keep_prob=keep_prob)

#Layer 3
Layer3_out = tf.nn.relu((tf.matmul(layer_flat_dp,l3_f_w) + (tf.matmul(z_,l2_f_w))) + l3_f_b)
Layer3_out_dp = tf.nn.dropout(Layer3_out,keep_prob=keep_prob)

#Layer 4
Layer4_out = (tf.nn.relu(tf.matmul(Layer3_out_dp,l4_f_w) + l4_f_b))
Layer4_out_dp = tf.nn.dropout(Layer4_out,keep_prob=keep_prob)

#Layer 5
Layer5_out = (tf.nn.relu(tf.matmul(Layer4_out_dp,l5_f_w) + l5_f_b))

#cost = tf.reduce_mean((( y_ * tf.log(s3)) + ((1 - y_) * tf.log(1.0 - s3))) * -1)
cost = tf.losses.mean_squared_error(y_,Layer5_out,weights=1.0)
accu,accu_op = tf.metrics.accuracy(y_,Layer5_out,weights=1.0)
...
cost_reff = sess.run(cost, feed_dict={x_: in_image_T, y_: out_image_T,z_: depth,keep_prob:1})
print("# Cost : ", cost_reff)

print('Traning Started')
print('0 #',end='')
for i in range(1000000):
    sess.run(train_step,feed_dict={x_:in_image_T,y_:out_image_T,z_: depth,keep_prob:0.9})
    print( '|',end='')
    ...
    if (i % 10 == 0) and (i > 0):
        ...
        if (i % 100 == 0) and (training_batch_size < 3999):
